...
ZIP_VER=`sed -n '8p' unikernel`
DEVICE_VER=`sed -n '4p' unikernel`
VER=`sed -n '6p' unikernel`
SU_VER=`sed -n '14p' unikernel`
KERNEL_NAME="$ZIP_VER$DEVICE_VER$VER"
TOOLCHAIN=`sed -n '12p' unikernel`
BUILD_CROSS_COMPILE=$TOOLCHAIN_DIR/$TOOLCHAIN


#vars
KERNEL_DEFCONFIG=exynos7420-zerofltespr_defconfig
BASE_VER=`sed -n '2p' unikernel`
DEVICE_VER=`sed -n '4p' unikernel`
VER=`sed -n '6p' unikernel`
...
{	
	...
	export KBUILD_BUILD_USER=unikernel
...
{	
	...
	if [ -e $BUILD_KERNEL_DIR/arch/arm64/boot/Image ]; then
	      ...
	      echo "Making zip ..."
	      cp $BOOTIMG $BUILD_KERNEL_DIR/build_image/zip_files
	      cd $FLASH_ZIP_FILES
